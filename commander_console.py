...
def main():
    ...
    parser.add_argument("--url", required=True, help="普羅米修斯系統的公開訪問網址 (由 Colab 提供)。")
    parser.add_argument("--symbol", type=str, default="TSLA", help="要分析的股票代碼。")
    parser.add_argument("--window", type=int, default=20, help="簡單移動平均線 (SMA) 的計算窗口期。")
    args = parser.parse_args()

    api_endpoint = f"{args.url.rstrip('/')}/api/v1/submit_task"

    task_payload = {"task_type": "simple_moving_average", "payload": {"symbol": args.symbol, "window": args.window}}
