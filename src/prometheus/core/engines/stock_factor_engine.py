...
class StockFactorEngine(BaseAnalyzer):
    ...
    async def _calculate_monthly_revenue_yoy(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            revenue_data = await self.finmind_client.get_monthly_revenue(
                stock_id, latest_date.year - 2, latest_date.month
            )
            ...
            def get_revenue_yoy(row):
                current_month_revenue = revenue_data[
                    (revenue_data.index.year == row["year"]) & (revenue_data.index.month == row["month"])
                ]

                last_year_month_revenue = revenue_data[
                    (revenue_data.index.year == row["year"] - 1) & (revenue_data.index.month == row["month"])
                ]
